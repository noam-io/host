...
module Releasy
  module Builders
    # Builds an OS X application bundle based on the one at https://github.com/trptcolin/ruby_app
    class NoamOsxApp < Builder
      ...
      def generate_tasks
        ...
        raise ConfigError, "#wrapper not valid wrapper: #{wrapper}" unless File.basename(wrapper) =~ VALID_WRAPPER

        desc "Build Noam Server OS X app"
        task "build:noam:osx:app" => folder
...
END
      ...
      def update_icon(app)
        if icon
          rm "#{app}/Contents/Resources/Ruby.icns", fileutils_options
