    ...
    ui.Views.GraphView = Backbone.View.extend({
        ...
        parseToD3: function() {
            ...
            d3.json(this.fakeData, function(data) {
                ...
                var nodes = _this.cluster.nodes(
                    _.filter(d,function(item){
                        return item.id
                    })
                );


//                links = _this.getEdges(data);

                // var path = _this.svg.selectAll("path.link")
                //     .data(nodes)
                //     .enter().append("svg:path")
                //     .attr("class", function(d) { return "link source-" + d.id + " target-" + d.id; })
                //     .attr("d", function(d, i) { return line(_this.spline[i]); });
 
                _this.svg.selectAll('g.node')
                    .data(nodes)
                    .enter().append("svg:g")
                    .attr('class','node')
                    .attr('id', function(d){ console.log(d); return "node-" + d.id});
        ...
        // Figure out what topics exist for what 
